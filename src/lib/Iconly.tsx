...
const icons: Record<string, React.MemoExoticComponent<(props: any) => JSX.Element>> = {
...
// Define the prop types for Iconly
interface IconlyProps {
  primaryColor?: string;
  secondaryColor?: string;
  set?: string;
  size?: number;
  stroke?: number;
  name: keyof typeof icons; // Restrict `name` to keys of the `icons` object
  [key: string]: any; // Allow additional props
}

const Iconly: React.FC<IconlyProps> = ({ primaryColor, secondaryColor, set, size, stroke, name, ...props }) => {
  const Icon = icons[name];
  if (!Icon) {
    throw new Error(`Icon '${name}' doesn't exist in react-native-iconly`);
  }
  return (
    <Icon
